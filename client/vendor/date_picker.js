...
function datePicker(e, elm) {
	...
	if(arguments[0]){
		...
		document.getElementById("my_calenda").appendChild(calDivObj);
		fBuildHtml();
	}
...
function fBuildHtml() {
	...
	calHtml += " <select name='tbSelYear' id='tbSelYear' style='font-size:9pt' onChange='fUpdateCal(0,0,1)'>";
	for(i=1970;i<2030;i++) calHtml += "<OPTION value='"+i+"'>"+i+"</OPTION>";
	calHtml += "</select>";
	calHtml += " <select name='tbSelMonth' id='tbSelMonth' style='font-size:9pt' onChange='fUpdateCal(0,0,1)'>";
...
function fUpdateCal(iYear, iMonth,stats) {

	if(stats == 1){
		iYear = $("#tbSelYear").val();
		iMonth = $("#tbSelMonth").val();
	}
